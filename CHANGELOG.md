...
- `README.md` prose polish: removed em dashes, tightened wording, and added the 330+ test suite breakdown (unit, public-surface contract, Hypothesis property tests). No code or behavior change. (#139)
...
- New `tests/test_cli.py::test_parser_accepts_global_debug_flag`: verifies `--debug` is a top-level flag and the `args.debug` attribute is reachable from any subcommand via `args.debug`. (#73)
